...
composite calculation, lane weight application.
"""
...
from vectis_intel.community.score import score_clusters, _compute_cluster_stats
...
class TestComputeClusterStats:
    def test_basic_stats(self, db):
        cid = _seed_cluster(db, "c1", "Cluster 1", "grc", 1.5)
...
class TestScoreClusters:
    def test_scoring_with_lane_weight(self, db, taxonomy):
        cid = _seed_cluster(db, "test_cluster_a", "Test A", "grc", 1.5)
        for i in range(10):
    ...
    def test_empty_cluster_skipped(self, db, taxonomy):
        _seed_cluster(db, "test_cluster_a", "Test A", "grc", 1.5)
    ...
    def test_normalization_across_clusters(self, db, taxonomy):
        ...
        cid_a = _seed_cluster(db, "test_cluster_a", "Test A", "grc", 1.5)
